...
def load_data()
    ...
    # CORREÇÃO Lista de todas as colunas de datas que precisam ser convertidas
    colunas_datas = [
        'Data da solicitação', 
...
with col_dt1
    data_inicio = st.date_input(Data Inicial, value=cinco_dias_atras, format=DDMMYYYY)
with col_dt2
    data_fim = st.date_input(Data Final, value=hoje, format=DDMMYYYY)
...
def formatar_data(data)
    # CORREÇÃO Adicionada validação hasattr para garantir que é uma data válida que aceita formatação
...
def render_card(row, coluna)
    with st.container(border=True)
        ...
        if pd.isna(detalhe)
            detalhe = '-'
        st.caption(fDetalhe {detalhe})
        st.caption(fObra {row.get('Obra', '-')})

        if insumo in coluna
            qtd_sol = converter_numero(row.get('Quantidade solicitada', 0))
            un = row.get('Unidade de movimento', '')
            st.text(fQtd {qtd_sol,.2f} {un}.replace(',', 'X').replace('.', ',').replace('X', '.'))
            st.text(fNº Solicitação {row.get('Nº da Solicitação', '-')})
            ...
            with st.expander(Ver Detalhes)
                st.write(fData da solicitação {formatar_data(row.get('Data da solicitação'))})
                if 'Data autorização da solicitação' in row and pd.notna(row['Data autorização da solicitação']) and coluna != insumo_nao_aut
                    st.write(fData Aut. Solicit. {formatar_data(row['Data autorização da solicitação'])})

        elif pedido in coluna
            qtd_pendente = converter_numero(row.get('Saldo', 0))
            un = row.get('Unidade de movimento', '')
            st.text(fQtd pendente {qtd_pendente,.2f} {un}.replace(',', 'X').replace('.', ',').replace('X', '.'))
            st.text(fPedido {row.get('N° do Pedido', '-')})

            status = row.get('Situação do pedido', 'Pendente')
            st.markdown(forange[ {status} ])

            with st.expander(Ver Detalhes)
                st.write(fNº da Solicitação {row.get('Nº da Solicitação', '-')})
                st.write(fFornecedor {row.get('Fornecedor', '-')})
                st.write(fData da solicitação {formatar_data(row.get('Data da solicitação'))})
                st.write(fData do pedido {formatar_data(row.get('Data do pedido'))})
                if coluna == pedido_aut
                    st.write(fData Aut. Pedido {formatar_data(row.get('Data autorização do pedido'))})

        elif entrega in coluna
            st.text(fPedido {row.get('N° do Pedido', '-')})
            qtd_ent = converter_numero(row.get('Quantidade entregue', 0))
            un = row.get('Unidade de movimento', '')
            st.text(fQtd entregue {qtd_ent,.2f} {un}.replace(',', 'X').replace('.', ',').replace('X', '.'))
